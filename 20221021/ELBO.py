...
def init_model(n_train):
	...
	gpflow.utilities.print_summary(m_svgp_white)
...
def run_optim(model, iterations):
	...
	for step in range(iterations):
		...
		if model.name == "svgp" and model.q_mu.trainable == False:
			...
			print(model.name, model.kernel.variance.numpy())
			elbo = model.maximum_log_likelihood_objective(data).numpy()
			variance.append(model.kernel.variance.numpy())
			logf.append(elbo)
...
for train_index, test_index in kf.split(X):
	...
	for m in mods:
		print(m)
		t0 = time.time()
		var_i, logf_i, nlpd_i = run_optim(m, maxiter)
		t = time.time() - t0
...
var_i_mean = np.mean(VAR_i, 2)

plt.figure(1)
plt.title("ELBO" + "_" + data_name)
plt.plot(var_i_mean[0, :][:], elbo_mean[0, :][:], "r-o", label=names[0])
plt.plot(var_i_mean[1, :][:], elbo_mean[1, :][:], "b-d", label=names[1])
plt.plot(var_i_mean[2, :][:], elbo_mean[2, :][:], "k-D", label=names[2])
# plt.xlim([0,1])
plt.legend()

plt.figure(2)
plt.title("NLPD" + "_" + data_name)
plt.plot(var_i_mean[0, :][:], nlpd_i_mean[0, :][:], label=names[0])
plt.plot(var_i_mean[1, :][:], nlpd_i_mean[1, :][:], label=names[1])
plt.plot(var_i_mean[2, :][:], nlpd_i_mean[2, :][:], label=names[2])
# plt.xlim([0,1])
plt.legend()
